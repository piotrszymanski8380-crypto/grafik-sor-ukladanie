...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const ostrzezenia = grafik.ostrzezeniaMiesiaca(wpisyPary, pracownicy, rok, miesiac, grafik.DOMYSLNE_PARAMETRY, wpisyRoczne);
  ...
  if (req.method === 'POST') {
    ...
    if (akcja === 'zapisz') {
      ...
      const powod = grafik.blokada(pracownik, data, kodEfektywny, wpisyPary, grafik.DOMYSLNE_PARAMETRY);
      if (powod) {
    ...
    if (akcja === 'generuj') {
      ...
      const wpisyPary = await wpisyOkresu(rok, miesiac, stanBiezacy.wpisy);
      const { noweWpisy, braki } = grafik.generujBrakujaceDyzury(wpisyPary, pracownicy, rok, miesiac, grafik.DOMYSLNE_PARAMETRY);
